...
export default class ApplyMajorController extends AbstractController<ICourses> {
    ...
    private async validate(req: any) {
        let { registation_start, registation_end, apply_majors, shifts, course_start, course_end } = req.body
        let [sDate, eDate] = CommonUtil.parseStartDateEndDate(registation_start, registation_end);
        let [cStart, cEnd] = CommonUtil.parseStartDateEndDate(course_start, course_end);
        if (cStart < sDate) {
    ...
    async update(req: any) {
        ...
        if (getData.course_start < new Date()) {
            if (getData.course_start.setHours(0,0,0,0) != data.course_start.setHours(0,0,0,0)) {
                if (req.body._user.roles.permissions.indexOf(pAdmin.adminAction.change_course_date) == -1) {
                    this.throwHttpError("វគ្គបានចាប់ផ្តើម អ្នកមិនមានសិទ្ធផ្លាស់ប្ដូរថ្ងៃចូលរៀនទេ!")
               }
            }
        }
        let obj = CommonUtil.removeKeys(data, ["_id", "status", "schools", "staffs"]);
    ...
    async setActive(req: any) {
        ...
        let data = await this.model.findOneAndUpdate(query, { $set: { status: targetStatus } }, { new: true });
    ...
    async setArchive(req: any) {
        ...
        let data = await this.model.findOneAndUpdate(query, { $set: setData }, { new: true });
        return this.checkThrowNotFound(data);
    }
            ...
            { $sort: { sort_condition: 1, _id: -1 } },
            {
                $facet: {
                    result: [
                        ...
                        { $project: { __v: 0, updatedAt : 0, requirement: 0, fee: 0, students: 0, student_amount: 0 }}, 
                    ],
                    totalCount: [
                        {
                            $count: 'count'
