...
export default class Controller extends AbstractController<IStudents> {
    ...
    async getList(req: any) {
        ...
        let { search, gender, schools, status}: any = req.query;
        let query: any = {
        ...
        if (status) {
            if (status == EnumConstant.REQUESTING) {
                matchStatus.$or = [
                    { verify_status : Number(status) },
                    { verify_status : null },
                ]
            } else {   
                matchStatus.verify_status = Number(status);   
            }
        }
        let matchSearch: any = {};
        if (search) {
                ...
                $facet: {
                    result: [
                        ...
                        {$unwind: {path: "$courses"}},
                        {
                            $addFields: {
                                status: {
                                    $cond: [{ $ifNull: ["$verify_status", false] }, "$verify_status", EnumConstant.REQUESTING]
                                },     
    ...
    async applyRequest(req: any) {
        ...
        let query: any = {
            _id: students,
            status: EnumConstant.ACTIVE, 
            verify_status: EnumConstant.REJECTED,
        }
        if (req.body._user.schools) {
        ...
        let reqs = new this.models.requestTimeline({
            ...
            timeline_type: EnumConstant.TimelineType.VERIFY,
            staffs: req.body._user._id
        });
        return createSession(async (session) => {
            await this.models.requestTimeline.create([reqs], { session: session });
            return await this.model.findOneAndUpdate({_id: getStudent._id}, {$set: { verify_status: EnumConstant.REQUESTING}}, {session: session, new:true})
        });
    }
    async approval(req: any) {
        ...
        return createSession(async (session) => {
            ...
            return await this.model.findOneAndUpdate({_id: getStudent._id}, {$set: {verify_status: status}}, {session: session, new:true})
        });
    }
    ...
    async getDetail(req: any, queryPoor: boolean = true) {
        ...
        if (getStudent.verify_status == EnumConstant.REJECTED) {
        ...
        response.status = response.verify_status == null ? EnumConstant.REQUESTING: response.verify_status;
        return response;
    }
}